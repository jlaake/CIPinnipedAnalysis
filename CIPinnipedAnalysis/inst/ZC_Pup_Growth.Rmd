...
```{r,results='asis',echo=FALSE,size='small'}
pander(res.adjust$model.table,digits=7)
```
...
```{r,echo=FALSE,fig.width=6,fig.height=7,dpi=600}
par(mfrow=c(2,1))
...
Figure 1a. Predicted Zalophus pup weights on San Miguel Island at 1 Oct expressed as anomaly from overall mean. 
...
Figure 2a. Predicted Zalophus pup weights on San Miguel Island at 1 Feb expressed as anomaly from overall mean. 
...
Figure 1b. Predicted Zalophus pup weights on San Miguel Island at 1 Oct. 
...
Figure 2b. Predicted Zalophus pup weights on San Miguel Island at 1 Feb. 



```{r,echo=FALSE,fig.width=6,fig.height=7,dpi=600}
source(file.path(system.file(package="CIPinnipedAnalysis"),"ZC_Growth.r"))
...
par(lty=1)
plot_growth.series(sort(unique(pp$cohort)),female.averages,ylim=c(0,.12),xaxp=c(1988,2016,7))
par(lty=2)
plot_growth.series(sort(unique(pp$cohort))+.2,male.averages,pch=2,add=TRUE,slty=1)
...
```
```{r,echo=FALSE,fig.width=6,fig.height=7,dpi=600}
growth=data.frame(Year=sort(unique(pp$cohort)),female=female.averages$fit,male=male.averages$fit)
SSTDF=data.frame(Year=do.call(rbind,strsplit(do.call("rbind",strsplit(row.names(coef(res.environ[[4]])),"/"))[,2]," "))[,1])
SSTDF=merge(SSTDF,subset(env.data,select=c("Year","SST1")),by="Year")
dm=coef(res.environ[[res.environ$best]])[,c(5,7:8)]
fgr=vector("numeric",length=nrow(dm))
for( i in 1:nrow(dm))
  fgr[i]=as.matrix(dm[i,,drop=FALSE])%*%c(1,SSTDF$SST1[i],1)
SSTDF$fgr=fgr
########################################
# Sharon - new code here
#
fgr.errors=matrix(0,nrow=nrow(dm),ncol=length(res.environ.coef))   
for(j in 1:length(res.environ.coef))
{
   dm=res.environ.coef[[j]][,c(5,7:8)]
   for(i in 1:nrow(dm))
      fgr.errors[i,j]=as.matrix(dm[i,,drop=FALSE])%*%c(1,SSTDF$SST1[i],1)
}
SSTDF$fgr.se=sqrt(apply(fgr.errors,1,var))
# to here
##########################################
SSTDF=unique(SSTDF)
gr_comparison=merge(growth,SSTDF,by="Year")
```
Figure 2c. Daily growth rate of Zalophus pup weights on San Miguel Island initially captured and measured in Sept-Oct and recaptured and measured in late Jan to early Feb. This graph excludes weights from Dec 2014.
 

```{r,echo=FALSE,fig.width=6,fig.height=7,dpi=600}
with(gr_comparison,plot(female,fgr,ylab="Cross-sectional growth",xlab="Longitudinal growth"))
```
Figure 2d. Comparison of annual female pup rates from cross-sectional data with the environment model and longitudinal measuments of growth rate with tagged female pups measured in October and February.

```{r,echo=FALSE,fig.width=6,fig.height=7,dpi=600}
plot(as.numeric(as.character(SSTDF$Year)),SSTDF$fgr,type="b",ylab="Female pup growth rate (kg/day)",xlab="Birth Year",xaxp=c(1975,2016,41))
```
Figure 2e. Annual female pup growth rates predictions from cross-sectional data with the environment model.


```{r,echo=FALSE,fig.width=6,fig.height=7,dpi=600}
# Sharon - different plot fct to show 95% CI
plotCI(SSTDF$SST1,SSTDF$fgr,1.96*SSTDF$fgr.se,1.96*SSTDF$fgr.se,ylab="Female pup growth rate (kg/day)",xlab="Average sea surface temperature anomaly from Oct-Feb")
```
Figure 2f. Annual female pup growth rates predictions from cross-sectional data with the environment model as a function of average sea surface temperature anomaly from Oct-Feb.
...
pander(res.environ$model.table[1:15,],digits=7)
```
...
pander(res.environ.abun$model.table[1:15,],digits=7)
```
...
pander(res.environ.abun.diet$model.table[1:15,],digits=7)
```
...
```{r,results='asis',echo=FALSE,size='small'}
pander(res.environ.diet$model.table[1:15,],digits=7)
```
